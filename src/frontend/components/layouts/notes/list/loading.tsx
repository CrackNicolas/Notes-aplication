        ...
        <Fragment>
            {
                Array.from(Array(count).keys(), n => n + 1).map((index: number) => {
                    return (
                        <div key={index} title="Loading de nota" className={`animate-pulse group grid grid-cols-9 w-full bg-sixth pl-2.5 py-2 cursor-pointer rounded-md border-[0.1px] border-tertiary border-opacity-20`}>
                            <div className="col-span-7 md:col-span-8 flex flex-col gap-y-1 justify-between">
                                <span className="bg-tertiary opacity-20 w-[50%] h-[13px] rounded-full"></span>
                                <p className="bg-tertiary opacity-20 w-[calc(100%-60px)] h-[12px] rounded-full"></p>
                            </div>
