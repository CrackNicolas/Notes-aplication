    ...
    return (
        <ComponentModal open={open} setOpen={setOpen}>
            <div className={`relative flex flex-col items-center border-[0.1px] border-${color(status)} border-opacity-50 rounded-lg gap-y-5 px-3 sm:px-5 py-5`}>
                ...
                <button onClick={() => setOpen(false)} className={`outline-none rounded-full w-[200px] bg-primary hover:opacity-100 opacity-70 cursor-pointer text-${color(status)} border-[0.1px] border-${color(status)} `}>
                    Aceptar
                </button>
            </div>
        </ComponentModal>
