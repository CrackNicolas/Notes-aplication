...
export default function ComponentSelect(props: Props) {
    ...
    const { error, select_category, setSelect_category, register, required, setValue = () => { }, clearErrors = () => { }, style = { border: 'border-secondary', text: 'text-secondary', bg: 'bg-secondary' } } = props;
        ...
        <div ref={ref_select} className='relative flex w-full'>
            {
                (categorys.length === 0) ?
                    ...
                    :
                    <Fragment>
                        <div title="Categoria" onClick={() => setOpen_category(!open_category)} {...register('category', validation('category', required))} className={`flex justify-between items-center bg-primary w-full rounded-md border-[0.1px] ${open_category && 'rounded-b-none'} ${option_selected && 'border-secondary'} ${!error ? style.border : 'border-error'} border-opacity-50 py-1 px-2 cursor-pointer`}>
                            <span title="Seleccionar categoria" className={`${option_selected && 'text-secondary'} ${!error ? style.text : 'text-error'} text-md`}>
                                {select_category.title}
                            </span>
                            <ComponentIcon name={open_category ? 'caret-up' : 'caret-down'} size={20} description_class={`${option_selected && 'text-secondary'} ${!error ? style.text : 'text-error'} cursor-pointer`} />
                        </div>
                        <ul ref={list} title="Lista de categorias" className={`${(!open_category) && 'hidden'} absolute z-10 mt-[32px] w-full ${(categorys.length >= 4) && `h-[${categorys.length * 37}px]`} bg-primary border-[0.1px] ${!error ? `${style.border} scroll-select` : 'border-error scroll-select-error'} rounded-b-md border-opacity-50 overflow-hidden`}>
                            {
                                categorys.filter(category => category.title != select_category.title).map(category => {
                                    return (
                                        <li key={category.title} title={category.title} onClick={() => selected(category)} className={`flex justify-between items-center group ${!error ? `${style.text} hover:${style.bg}` : 'text-error hover:bg-error'} hover:text-primary px-2 py-1 cursor-pointer hover:font-semibold`}>
                                            <span className="text-md font-normal">
                                                {category.title}
                                            </span>
                                            <ComponentIcon name={category.icon} size={17} view_box="0 0 16 16" description_class={`group-hover:text-primary ${!error ? style.text : 'text-error'} duration-500 `} />
                                        </li>
                                    )
                                })
                            }
