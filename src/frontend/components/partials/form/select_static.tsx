...
export default function ComponentSelect(props: Props) {
    const { title, select, setSelect, items, style = { text: 'text-secondary', border: 'border-secondary', bg: 'bg-secondary' } } = props;
    ...
    return (
        <div ref={ref_select} className='relative flex w-full'>
            <div title={title} onClick={() => setOpen(!open)} className={`flex justify-between items-center bg-primary w-full rounded-md border-[0.1px] ${open && 'rounded-b-none'} ${option_selected && 'border-secondary'} ${style.border} border-opacity-50 py-1 px-2 cursor-pointer`}>
                <span title={`Seleccionar ${title}`} className={`${option_selected && 'text-secondary'} ${style.text} text-md`}>
                    {select}
                </span>
                <ComponentIcon name={open ? 'caret-up' : 'caret-down'} size={20} description_class={`${option_selected && 'text-secondary'} ${style?.text} cursor-pointer`} />
            </div>
            <ul ref={list} title="Lista de opciones" className={`${(!open) && 'hidden'} absolute z-10 mt-[32px] w-full bg-primary border-[0.1px] ${style.border} rounded-b-md border-opacity-50`}>
                {
                    select != `${title}...` && (
                        <li onClick={() => selected(`${title}...`)} title={`${title}...`} className="group flex justify-between hover:bg-secondary items-center px-2 py-1 cursor-pointer hover:font-semibold">
                            <span className={`text-md font-normal group-hover:font-semibold group-hover:text-primary ${style.text}`}>
                                {title}...
                            </span>
                        </li>
                    )
                    ...
                    items.map((item: Props_items_select, index: number) => {
                        return (
                            <li key={index} onClick={() => selected(item.value)} title={item.value} className="group flex justify-between items-center hover:bg-secondary px-2 py-1 cursor-pointer">
                                <span className={`text-md font-normal group-hover:text-primary ${style.text}`}>
                                    {item.value}
                                </span>
